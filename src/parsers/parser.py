...
def parse_raw(rawfile):
    ...
    for gen in data.raw.get_generators():
        # adding the slack generator as a generator class for IBR MPC simulation
        if False:#gen.i in data.raw.slack_buses:  # --- slack generators ---
            ...
            # we want to add the slack generators
            if (g.stat == True):#(g.i in PV_list) and (g.stat == True):
